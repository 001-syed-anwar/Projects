  ...
  if($filetype =="quotation"){
    ...
    foreach ($textPos as $variableName => $textInfo) {
      ...
      //  list($x, $y, $fontSize, $color) = explode('/', $textInfo);
        ...
        $x = isset($textInfoParts[0]) ? intval($textInfoParts[0]) : 0; 
        $y = isset($textInfoParts[1]) ? intval($textInfoParts[1]) : 0; 
        $fontSize = isset($textInfoParts[2]) ? intval($textInfoParts[2]) : 12; 
...
<div class="row my-5 justify-content-center" >
  ...
  if (!empty($data)) {
    foreach ($data as $poster) {
        ...
        if($filetype=="poster" && $extension=="jpeg" || $extension=="jpg"){
            $path="./poster";
        }
        else{
            $path="./quotation";
  ...
  <div class="mb-3">
   ...
   <select name="type" id="type" class="form-select">
       <?php 
        $sql="SELECT DocType from postermaster WHERE posterID=$id";
